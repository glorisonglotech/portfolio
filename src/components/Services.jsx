...
const severityColor = {
  CRITICAL: { dot: "bg-red-500", text: "text-red-400", border: "border-red-500/40", badge: "bg-red-500/10" },
  HIGH:     { dot: "bg-orange-400", text: "text-orange-400", border: "border-orange-500/30", badge: "bg-orange-500/10" },
  MEDIUM:   { dot: "bg-yellow-400", text: "text-yellow-400", border: "border-yellow-500/30", badge: "bg-yellow-500/10" },
};

// Animated scan line for cyber cards
const ScanLine = () => (
  <motion.div
...
export const Services = () => {
  ...
  return (
    <section id="services" className="py-20 bg-secondary/20 relative overflow-hidden">
      ...
      {/* Scan-line overlay matching hero */}
      <div
      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        >
          {/* Terminal badge */}
          <div className="inline-flex items-center gap-2 bg-black/40 border border-red-500/30 rounded-full px-4 py-1.5 mb-5 backdrop-blur-sm">
        ...
        <div className="mb-16">
          ...
          >
            <div className="flex items-center gap-2 bg-black/40 border border-green-500/30 rounded-lg px-3 py-1.5 backdrop-blur-sm">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6">
            {mernServices.map((service, index) => {
              ...
              return (
                ...
                >
                  <div className={`
                    relative h-full rounded-xl border bg-black/30 backdrop-blur-sm p-5
                    transition-all duration-300 overflow-hidden
                    ${isHovered
                      ? "border-red-500 shadow-lg shadow-red-500/20 bg-black/50"
                  ...
                  `}>
                    ...
                    {/* Corner brackets */}
                    <span className="absolute top-2 left-2 text-red-500/30 font-mono text-xs group-hover:text-red-500/60 transition-colors">[</span>
                    <span className="absolute top-2 right-2 text-red-500/30 font-mono text-xs group-hover:text-red-500/60 transition-colors">]</span>
                    ...
                    `}>
                      <Icon className={`h-5 w-5 transition-colors duration-300 ${isHovered ? "text-white" : "text-red-400"}`} />
                    </div>

                    {/* Tech tag */}
                    <div className="mb-2">
                      <span className="font-mono text-xs text-red-400/70 tracking-widest">{service.tag}</span>
                    </div>

                    <h3 className="font-bold text-sm sm:text-base mb-2 text-foreground">{service.title}</h3>
        ...
        <div>
          <motion.div
            ...
            className="flex items-center gap-3 mb-8"
          >
            <div className="flex items-center gap-2 bg-black/40 border border-red-500/30 rounded-lg px-3 py-1.5 backdrop-blur-sm">
              <Shield className="h-5 w-5 text-red-400" />
              <span className="font-mono text-sm text-red-400 font-semibold tracking-wide">
                Cybersecurity Services
              </span>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
            {cybersecurityServices.map((service, index) => {
              ...
              const sc = severityColor[service.severity];
              return (
                <motion.div
                  key={index}
                  initial={{ opacity: 0, y: 20 }}
                ...
                >
                  <div className={`
                    ...
                    transition-all duration-300 overflow-hidden
                    ${isHovered
                      ? `${sc.border} shadow-lg bg-black/50`
                      : "border-red-500/20 hover:border-red-500/40"}
                  `}
                    style={isHovered ? { boxShadow: `0 0 20px -5px rgba(239,68,68,0.25)` } : {}}
                  >
                    ...
                    <div className="flex items-start justify-between mb-3 mt-2">
                      ...
                      <div className={`flex items-center gap-1.5 px-2 py-1 rounded-md border ${sc.border} ${sc.badge}`}>
                        <span className={`inline-block w-1.5 h-1.5 rounded-full ${sc.dot} ${service.severity === "CRITICAL" ? "animate-pulse" : ""}`} />
                    ...
                    <h3 className="font-bold text-sm sm:text-base mb-2 text-foreground leading-snug">{service.title}</h3>
                    <p className="text-xs sm:text-sm text-muted-foreground leading-relaxed">{service.description}</p>

                    {/* Bottom status bar */}
                    <div className={`mt-4 pt-3 border-t border-red-500/10 flex items-center gap-2`}>
                      <span className="inline-block w-1.5 h-1.5 rounded-full bg-green-400" />
                      <span className="font-mono text-xs text-green-400/70">service.active</span>
                    </div>
        ...
        <motion.div
          ...
          className="mt-14 flex flex-wrap items-center justify-center gap-4 sm:gap-8 border-t border-red-500/10 pt-8"
        >
          {[
            { label: "Services Active", value: "10", dot: "bg-green-400" },
            { label: "Threats Mitigated", value: "10+", dot: "bg-red-500" },
            { label: "Uptime", value: "99.9%", dot: "bg-blue-400" },
          ].map((item) => (
